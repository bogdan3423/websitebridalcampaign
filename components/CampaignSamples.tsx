...
export function CampaignSamples() {
  ...
  return <div className="campaign-samples">
    ...
    {tabs.map((tab, index) => <div key={tab} className="sample-panel" id={"sample-panel-" + index} role="tabpanel" aria-labelledby={"sample-tab-" + index} tabIndex={0} hidden={active !== index}>
      {active === 0 && index === 0 && <div className="sample-posts">{posts.map((post) => <figure key={post.day} className={"sample-post sample-post-" + post.photo}><div className="post-art"><div className="post-photo"><Image src={gallery[post.photo].src} alt={gallery[post.photo].alt} fill sizes="(max-width: 650px) 85vw, 28vw" /></div><span className="post-art-title">{post.title}</span></div><figcaption><details><summary aria-label={post.day + " — textul postării"}>{post.day}<span aria-hidden="true">+</span></summary><p>{post.copy}</p></details></figcaption></figure>)}</div>}
